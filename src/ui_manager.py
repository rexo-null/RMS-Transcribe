...
from utils import ensure_dirs, export_result_files, format_time, is_supported_audio, make_logger
...
class UIManager:
    ...
    def on_export_json(self):
        ...
        saved = export_result_files(result, Path(save_path).parent)
        target = Path(save_path)
        target.write_text(saved["json"].read_text(encoding="utf-8"), encoding="utf-8")
    ...
    def on_export_txt(self):
        ...
        saved = export_result_files(result, Path(save_path).parent)
        target = Path(save_path)
        target.write_text(saved["txt"].read_text(encoding="utf-8"), encoding="utf-8")
