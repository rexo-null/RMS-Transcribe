...
Пожалуйста, получите токен и введите его в поле ниже."""
        ...
        self.dialog.bind("<Control-v>", self._paste_from_clipboard)
    ...
    def _paste_from_clipboard(self, event=None):
        ...
        try:
            ...
            if clipboard_text:
                ...
                # Вставляем текст в позицию курсора или заменяем выделение
                self.token_entry.insert("insert", clipboard_text)
        except Exception:
            pass
        return "break"
    ...
    def _show_context_menu(self, event):
        ...
        context_menu.add_command(label="Вставить", command=self._paste_from_clipboard)
        context_menu.add_separator()
        context_menu.add_command(label="Вырезать", command=lambda: self.token_entry.event_generate("<<Cut>>"))
        context_menu.add_command(label="Копировать", command=lambda: self.token_entry.event_generate("<<Copy>>"))
